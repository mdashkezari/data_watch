...
@router.get(
            ...
            tags=[], 
            status_code=status.HTTP_200_OK,
            summary="Search for mismatch between catalogs on different servers",
            description="",
            response_description=RESPONSE_MODEL_DESCIPTION,
            response_model=RESMOD
            )
async def catalogs_mismatch(response: Response):
    """
    Return a list of discrepancies between the catalog dataframe generated by different servers.
    """
    try:
        mismatchDF, msg, err = pd.DataFrame({}), "", False
        mismatches = []
        catalogs = {server:query("select * from dbo.udfCatalog()", servers=[server])[0] for server in SERVERS}

        propsDF = pd.DataFrame({"server": [], "rowCount": [], "colCount": []})
        for server, catalog in catalogs.items(): 
            propsDF.loc[len(propsDF.index)] = [server, len(catalog), len(catalog.columns)] 
        if len(set(propsDF["rowCount"].values)) > 1:
            mismatches.append("inconsistent row numbers. " + ", ".join([f"{z[0]}: {z[1]}" for z in zip(propsDF["server"].values, propsDF["rowCount"].values)]) )
        if len(set(propsDF["colCount"].values)) > 1:
            mismatches.append("inconsistent column numbers. " + ", ".join([f"{z[0]}: {z[1]}" for z in zip(propsDF["server"].values, propsDF["colCount"].values)]) )

        refServer = "rainier"    
        for col in catalogs[refServer].columns:
            for s1, c1 in catalogs.items():
                for s2, c2 in catalogs.items():
                    if s1 == s2: continue
                    if not c1[col].equals(c2[col]):
                        mismatches.append(f"inconsistent column values. check {col} column in {s1} and {s2} servers.")

        mismatchDF = pd.DataFrame({"mismatch": mismatches})
        msg = SUCCESS_MSG
    except Exception as e:
        response.status_code = status.HTTP_500_INTERNAL_SERVER_ERROR
        ...
        err = True
        print(msg)        
    return {"data": mismatchDF.to_dict(), "message": msg, "error": err, "version": API_VERSION}